...
export default function ExceptionsPage() {
  ...
  const unresolvedExceptions = filtered.filter((e: any) => e.resolutionState !== 'RESOLVED');
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">Exceptions Central Workspace</h1>
          <p className="text-xs text-slate-400 mt-0.5">Review, investigate, and resolve financial discrepancies</p>
        </div>
        <span className="px-3 py-1.5 rounded-xl bg-rose-500/20 text-rose-300 border border-rose-500/30 text-xs font-bold flex items-center gap-2">
          <AlertTriangle className="w-4 h-4 flex-shrink-0" />
          <span>{filtered.filter((e: any) => e.resolutionState !== 'RESOLVED').length} Unresolved Exceptions</span>
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        {loading ? (
          <p className="text-xs text-slate-500 col-span-2 text-center py-12">Loading exception records...</p>
        ) : unresolvedExceptions.length === 0 ? (
          <p className="text-xs text-slate-550 col-span-2 text-center py-12 bg-slate-900/10 rounded-2xl border border-slate-850 border-dashed">
            All exceptions in this category have been resolved!
          </p>
        ) : (
          unresolvedExceptions.map((item) => (
            <div
              key={item.id}
              className="glass-panel rounded-2xl p-6 border border-slate-800 flex flex-col justify-between space-y-4 hover:border-indigo-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-indigo-500/2 shadow-md relative group/card"
            >
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-indigo-400 group-hover/card:text-indigo-300 transition-colors flex items-center gap-1">
                  ...
                  <div className="text-xs text-slate-350 space-y-1 font-mono bg-slate-900/60 p-3 rounded-xl border border-slate-850">
              ...
              <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between gap-3 relative z-10">
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handleResolve(item.id, item.resolutionState === 'RESOLVED' ? 'KEPT_OPEN' : 'RESOLVED')}
                  ...
                  >
                    ...
                    <span>{item.resolutionState === 'RESOLVED' ? 'Resolved' : 'Mark Resolved'}</span>
                  </button>

                  <button
                    onClick={() => handleResolve(item.id, 'KEPT_OPEN')}
                    className="px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-xs font-semibold text-slate-400 hover:text-slate-200"
                  >
                    Keep Open
                  </button>
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex justify-end">
          <div className="w-full sm:max-w-md lg:max-w-lg bg-[#0d1322] h-full p-4 sm:p-8 overflow-y-auto touch-scroll border-l border-slate-800 shadow-2xl flex flex-col justify-between">
            ...
            <div className="pt-6 border-t border-slate-800 flex items-center gap-3">
              <button
                onClick={() => handleResolve(selectedException.id, selectedException.resolutionState === 'RESOLVED' ? 'KEPT_OPEN' : 'RESOLVED')}
              ...
              >
                ...
                <span>{selectedException.resolutionState === 'RESOLVED' ? 'Mark Unresolved' : 'Mark Resolved'}</span>
              </button>
              <Link
