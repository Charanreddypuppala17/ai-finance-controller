  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Reconciliation & Audit Reports</h1>
          <p className="text-xs text-slate-400 mt-1">Export official compliance, summary, and audit reports in CSV/PDF format</p>
        </div>
        <button
          onClick={handleExportCsv}
          disabled={!reportData?.transactions || reportData.transactions.length === 0}
          className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold text-xs transition-all shadow-lg shadow-indigo-600/20 flex items-center gap-2"
        >
